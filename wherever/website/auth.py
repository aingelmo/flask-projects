...
from website.extensions import client
from website.forms import LoginForm, RegistrationForm
from website.models import GoogleUsers, Users, db
...
def callback():
    ...
    exists = (
        db.session.query(GoogleUsers.id).filter_by(email=users_email).first()
        is not None
    )
